...
namespace Retronix_Inventory_System_Management.Models
{
    ...
    public class Dashboard
    {
        ...
        {
            using (SqlConnection connection = new SqlConnection("Data Source=.\\SQLEXPRESS;Initial Catalog=Inventory Database;Integrated Security=False;User Id=sa;Password=qqq555"))
            {
                connection.Open();
            ...
            using (SqlConnection connection = new SqlConnection("Data Source=.\\SQLEXPRESS;Initial Catalog=Inventory Database;Integrated Security=False;User Id=sa;Password=qqq555"))
            {
                ...
                using (var command = new SqlCommand())
                {
                    ...
                    command.CommandText = @"select top 5 P.ProductName, sum(OrderItem.Quantity) as Q
                                            ...
                                            inner
                                            join [OrderTbl] O on O.OrderId = OrderItem.OrderId
                                            where OrderDate between @fromDate and @toDate
                    ...
                    command.CommandText = @"select ProductName, Stock
                                            from Product
                                            where Stock <= 2500 and IsDiscontinued = 0";
                    reader = command.ExecuteReader();
                    while (reader.Read())
                    {
                        UnderstockList.Add(
